...
inverse-transforms the scaled Close predictions back to real price units, and
prints RMSE, MAE and MAPE.
...
def main() -> int:
    ...
    for path in (test_path, scaler_path, model_path):
        if not path.exists():
            print(f"Hata: '{path}' bulunamadı. Önce preprocess.py ve train.py çalıştırılmalı.", file=sys.stderr)
            return 1
    ...
    print(f"Test seti örnek sayısı: {len(actuals)}")
